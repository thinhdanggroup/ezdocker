...
const buildDockerfile = (json) => {
  ...
  for (let i in listService) {
    switch (listService[i].type) {
      case "react-app":
        try {
          fs.unlinkSync("Dockerfile-" + nameDocker[i])
        } catch (e) { }
        ...
        reactAppTemplate.split("|")
          .forEach((line) => {
            ...
            fs.appendFile("Dockerfile-" + nameDocker[i], writeData + "\n", function (err) {
  ...
  try {
    fs.unlinkSync("build.sh")
  } catch (e) { }
  for (let i in listService) {
    let contentBuild = "docker build -t " + json.name + "-" + nameDocker[i] +
      " -f " + "Dockerfile-" + nameDocker[i] + " ."
    fs.appendFile("build.sh", contentBuild, function (err) {
      if (err) return console.log(err);
    });
  }
}
